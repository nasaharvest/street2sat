...
def upload():
    ...
    if form.validate_on_submit():
        ...
        for jpg in jpg_files:
            if (
                jpg.filename.split(".")[-1].lower() != "jpg"
                and jpg.filename.split(".")[-1].lower() != "jpeg"
            ):
...
def prediction():
    ...
    if form.validate_on_submit():
        ...
        for file in form.files.data[:20]:
            ...
            if not (
                (file_filename.split(".")[-1].lower() == "jpg")
                or (file_filename.split(".")[-1].lower() == "jpeg")
            ):
            ...
            try:
                ...
                flash(
                    "Latitude longitude could not be found for {}".format(file_filename)
                )
                return redirect(url_for("model.prediction"))
            try:
                ...
                flash(
                    "Time information could not be found for {}".format(file_filename)
                )
                return redirect(url_for("model.prediction"))
            try:
                ...
                flash(
                    "Focal length information could not be found for {}".format(
                        file_filename
                    )
                )
                return redirect(url_for("model.prediction"))
            try:
                ...
                flash(
                    "Image height/width information could not be found for {}".format(
                        file_filename
                    )
                )
                return redirect(url_for("model.prediction"))
...
def displayone():
    ...
    folium.Marker(
        lat_long,
        popup="<i> {}\n{}:{}:{}\nLocation:{}</i>".format(
            jpg_files[0], time.hour, time.minute, time.second, lat_long
        ),
        icon=folium.Icon(color="lightgray"),
    ).add_to(map)
    map_html = map._repr_html_()

    info = get_distance_meters(
        results, focal_length, pixel_width, pixel_height, lat_long
    )
    if info == {}:
        flash("No crops found for {}".format(jpg_files[0]))
    s = plot_labels(img, results)
    return render_template(
        "display.html", imgs=choose_picture, image=s, map=map_html, info=info
    )
...
def display():
    ...
    if choose_picture.validate_on_submit():
        ...
        map = folium.Map(
            location=file_coord[selected_img], zoom_start=17, tiles="OpenStreetMap"
        )
        for file in jpg_files:
            time = file_time[file]
        ...
        for file, crops in new_points.items():
            for crop, new_p in crops.items():
                folium.Marker(
                    new_p,
                    popup="<i> {}\n{}\n{}/i>".format(
                        file, file_distance[file], bearings[file]
                    ),
        ...
        if info == {}:
            ...
            link = "https://www.google.com/maps/search/?api=1&query={},{}".format(
                new_points[selected_img][crop_on_gm][0],
                new_points[selected_img][crop_on_gm][1],
            )
            return render_template(
                "display.html",
                imgs=choose_picture,
    ...
    for file in jpg_files:
        ...
        folium.Marker(
            lat_long,
            popup="<i> {}\n{}:{}:{}</i>".format(
                file, time.hour, time.minute, time.second
            ),
            icon=folium.Icon(color="lightgray"),
        ).add_to(map)
        file_time[time] = file
