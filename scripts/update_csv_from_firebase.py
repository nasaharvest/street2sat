...
def get_images_already_being_labeled():
	...
	assert len(dupes) == 0, "Found duplicates in images being labeled. One of the labeling tasks needs to be removed."
...
while True:
	...
	docs = coll.order_by('input_img').start_after({'input_img':name[-1]}).limit(limit_amount).stream()
	assert len(lat) == len(lon) == len(name) == len(being_labeled) == len(country) == len(test_set) == len(time_d) == len(focal_length) == len(pixel_height), 'lengths incorrect'
